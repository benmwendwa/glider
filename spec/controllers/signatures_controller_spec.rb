require 'spec_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

describe SignaturesController do

  # This should return the minimal set of attributes required to create a valid
  # Signature. As you add validations to Signature, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) { { "deal_id" => "MyString" } }

  # This should return the minimal set of values that should be in the session
  # in order to pass any filters (e.g. authentication) defined in
  # SignaturesController. Be sure to keep this updated too.
  let(:valid_session) { {} }

  describe "GET index" do
    it "assigns all signatures as @signatures" do
      signature = Signature.create! valid_attributes
      get :index, {}, valid_session
      assigns(:signatures).should eq([signature])
    end
  end

  describe "GET show" do
    it "assigns the requested signature as @signature" do
      signature = Signature.create! valid_attributes
      get :show, {:id => signature.to_param}, valid_session
      assigns(:signature).should eq(signature)
    end
  end

  describe "GET new" do
    it "assigns a new signature as @signature" do
      get :new, {}, valid_session
      assigns(:signature).should be_a_new(Signature)
    end
  end

  describe "GET edit" do
    it "assigns the requested signature as @signature" do
      signature = Signature.create! valid_attributes
      get :edit, {:id => signature.to_param}, valid_session
      assigns(:signature).should eq(signature)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new Signature" do
        expect {
          post :create, {:signature => valid_attributes}, valid_session
        }.to change(Signature, :count).by(1)
      end

      it "assigns a newly created signature as @signature" do
        post :create, {:signature => valid_attributes}, valid_session
        assigns(:signature).should be_a(Signature)
        assigns(:signature).should be_persisted
      end

      it "redirects to the created signature" do
        post :create, {:signature => valid_attributes}, valid_session
        response.should redirect_to(Signature.last)
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved signature as @signature" do
        # Trigger the behavior that occurs when invalid params are submitted
        Signature.any_instance.stub(:save).and_return(false)
        post :create, {:signature => { "deal_id" => "invalid value" }}, valid_session
        assigns(:signature).should be_a_new(Signature)
      end

      it "re-renders the 'new' template" do
        # Trigger the behavior that occurs when invalid params are submitted
        Signature.any_instance.stub(:save).and_return(false)
        post :create, {:signature => { "deal_id" => "invalid value" }}, valid_session
        response.should render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested signature" do
        signature = Signature.create! valid_attributes
        # Assuming there are no other signatures in the database, this
        # specifies that the Signature created on the previous line
        # receives the :update_attributes message with whatever params are
        # submitted in the request.
        Signature.any_instance.should_receive(:update_attributes).with({ "deal_id" => "MyString" })
        put :update, {:id => signature.to_param, :signature => { "deal_id" => "MyString" }}, valid_session
      end

      it "assigns the requested signature as @signature" do
        signature = Signature.create! valid_attributes
        put :update, {:id => signature.to_param, :signature => valid_attributes}, valid_session
        assigns(:signature).should eq(signature)
      end

      it "redirects to the signature" do
        signature = Signature.create! valid_attributes
        put :update, {:id => signature.to_param, :signature => valid_attributes}, valid_session
        response.should redirect_to(signature)
      end
    end

    describe "with invalid params" do
      it "assigns the signature as @signature" do
        signature = Signature.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Signature.any_instance.stub(:save).and_return(false)
        put :update, {:id => signature.to_param, :signature => { "deal_id" => "invalid value" }}, valid_session
        assigns(:signature).should eq(signature)
      end

      it "re-renders the 'edit' template" do
        signature = Signature.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Signature.any_instance.stub(:save).and_return(false)
        put :update, {:id => signature.to_param, :signature => { "deal_id" => "invalid value" }}, valid_session
        response.should render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested signature" do
      signature = Signature.create! valid_attributes
      expect {
        delete :destroy, {:id => signature.to_param}, valid_session
      }.to change(Signature, :count).by(-1)
    end

    it "redirects to the signatures list" do
      signature = Signature.create! valid_attributes
      delete :destroy, {:id => signature.to_param}, valid_session
      response.should redirect_to(signatures_url)
    end
  end

end
